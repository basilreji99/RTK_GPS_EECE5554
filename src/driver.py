...
def gps_driver_pub(port_id):
    ...
    port = serial.Serial(port_id, 57600, timeout=1.0)
    ...
    while not rospy.is_shutdown():
        ...
        if line == "":
            ...
            if line.startswith(b"$GNGGA") or line.startswith(b"\r$GNGGA"):
                ...
                if ll.count(b'') == 0 and len(ll) == 15:
                    time_stat, lat, lat_dir, lon, lon_dir, quality, alt = ll[1], ll[2], ll[3], ll[4], ll[5], int(ll[6]), float(ll[9])
                    ...
                    time = datetime.datetime(yr, mn, dt, hr, mn, se, ds).timestamp()
                    gps_msg_obj.Header.stamp = rospy.Time(time)
